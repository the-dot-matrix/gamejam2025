...
vec4 effect(vec4 color, Image tex, vec2 txy, vec2 sxy) {
  ...
  avg = vec4(avg.xyz/3, avg.w);
  float M = (avg.x+avg.y+avg.z)/3;
  avg = color * avg;
  vec4 bleed = vec4(M*avg.x,M*avg.y,(M/d)*(avg.z+(d/3)),avg.w);
  bleed = vec4(bleed.xyz/d, bleed.w);
  return (mx==0 && my==0) ? avg : bleed;
}
