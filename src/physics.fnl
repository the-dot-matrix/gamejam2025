...
(fn Physics.acc [v dt norm? tang?]
  (var anew nil)
  (let [z   (Physics.force Z)
        g   (Vec:new 0 z)
        i   (when norm? (Vec:new (Physics.force (norm?:polar) z)))
...
(fn Physics.pos [p v dt] (+ p (* v dt)))

(fn Physics.avg [vecs] (when (> (length vecs) 0)
